    ...
    : UI("##Content")
{
    ...
    //m_Tree->AddDynamic_RightClick(this, (UI_DELEGATE_1)&ContentUI::RightClickFunction);
...
void ContentUI::Reload()
{
    ...
    // 레벨 로딩
    LevelUI* level = (LevelUI*)ImGuiMgr::GetInst()->FindUI("##Level");
    level->m_vecLevelName.clear();
    ...
    for (size_t i = 0; i < m_vecResPath.size(); ++i)
    {
            ...
            //로딩 된 것들 중 레벨은 레벨UI로 옮긴다.
        {
            ...
            if (L".lv" == strExt) {
                level->m_vecLevelName.push_back(m_vecResPath[i]);
            }
            else {
                continue;
            }
...
//미구현
void ContentUI::RightClickFunction(DWORD_PTR _RightClickedNode)
{
    // RightClickNode를 가져온다.
    TreeNode* pSelectedNode = (TreeNode*)_RightClickedNode;
    CRes* pSelectObject = (CRes*)pSelectedNode->GetData();

    if (nullptr == pSelectObject)
        return;

    RES_TYPE type = pSelectObject->GetType();
    ...
    case RES_TYPE::MATERIAL:
       ...
       /* ImGui::Text("Edit name:");
        ...
        string EditedNameID = "##RightClickFunc" + EditedName;
        if (ImGui::InputText(EditedNameID.c_str(), (char*)EditedName.c_str(), sizeof(EditedName))) {
            const wstring& name = wstring(EditedName.begin(), EditedName.end());
            pSelectObject->

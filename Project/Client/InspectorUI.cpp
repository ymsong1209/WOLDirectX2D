	...
	, m_arrComUI{}
{
	...
	m_arrResUI[(UINT)RES_TYPE::PREFAB]->SetSize(0.f, 200.f);
	...
	m_arrComUI[(UINT)COMPONENT_TYPE::ANIMATOR2D]->SetSize(0.f, 300.f);
	...
	m_arrComUI[(UINT)COMPONENT_TYPE::FSMCOM] = new FSMUI;
	m_arrComUI[(UINT)COMPONENT_TYPE::FSMCOM]->SetSize(0.f, 150.f);
	AddChildUI(m_arrComUI[(UINT)COMPONENT_TYPE::FSMCOM]);

	m_arrComUI[(UINT)COMPONENT_TYPE::RIGIDBODY] = new RigidBodyUI;
	m_arrComUI[(UINT)COMPONENT_TYPE::RIGIDBODY]->SetSize(0.f, 250.f);
...
void InspectorUI::SetTargetObject(CGameObject* _Target, bool _isprefabObj)
{
	if (!_isprefabObj && _Target!=nullptr) {
		ClearTargetResource();
	}
	...
	for (size_t i = 0; i < m_vecScriptUI.size(); ++i)
	{
		...
		m_vecScriptUI[i]->SetSize(0.f, 150.f);
	}
...
void InspectorUI::SetTargetResource(Ptr<CRes> _Res)
{
	...
	if (type == RES_TYPE::PREFAB)
	{
		CPrefab* prefab = (CPrefab*)_Res.Get();
		SetTargetObject(prefab->GetProtoObject(), true);
	}
}

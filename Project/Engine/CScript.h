#pragma once
#include "CComponent.h"

#include "CLevelMgr.h"
#include "CLevel.h"
#include "CLayer.h"

#include "CResMgr.h"
#include "CTimeMgr.h"
#include "CKeyMgr.h"

#include "CGameObject.h"
#include "components.h"

// Define bitmasks for each direction
const int DIR_UP = 1 << 0;    // 0001 in binary
const int DIR_DOWN = 1 << 1;  // 0010 in binary 
const int DIR_LEFT = 1 << 2;  // 0100 in binary 
const int DIR_RIGHT = 1 << 3; // 1000 in binary 

enum class SCRIPT_PARAM
{
    INT,
    FLOAT,
    VEC2,
    VEC4,
    TEX
};

struct tScriptParam
{
    SCRIPT_PARAM    eParam;
    void*           pData;
    string          strDesc;
};

struct tScriptTexParam
{
    Ptr<CTexture>& tex;
    string strDesc;
};

class CCollider2D;
class CTexture;

class CScript :
    public CComponent
{
private:
    UINT                    m_iScriptType;
    vector<tScriptParam>    m_vecParam;
    vector<tScriptTexParam> m_vecTexParam;

public:
    void Destroy() { DestroyObject(GetOwner()); }
    void SetLifeSpan(float _Time) { GetOwner()->SetLifeSpan(_Time); }
    UINT GetScriptType() { return m_iScriptType; }
    const vector<tScriptParam>& GetScriptParam() { return m_vecParam; }
    vector<tScriptTexParam>& GetScriptTexParam() { return m_vecTexParam; }
    void SetScriptTexParam(Ptr<CTexture> pCurTex, const string& _Desc);
    void ChangeState(UINT _ScriptType, bool _isRepeat = false);

public:
    virtual void finaltick() final {};

    virtual void BeginOverlap(CCollider2D* _Other) {}
    virtual void OnOverlap(CCollider2D* _Other) {}
    virtual void EndOverlap(CCollider2D* _Other) {}

    virtual void Enter() {}
    virtual void Exit() {}
    

public:
    virtual void SaveToLevelFile(FILE* _File) override {}
    virtual void LoadFromLevelFile(FILE* _FILE) override {}

protected:
    void AddScriptParam(SCRIPT_PARAM eParam, void* _pData, const string& _Desc);
    void AddScriptTexParam(Ptr<CTexture>& Tex, const string& _Desc);

public:
    CScript(UINT _iScriptType);
    ~CScript();
};


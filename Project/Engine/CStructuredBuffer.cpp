...
{
	//Create를 다시 호출하면 기존에 있던 buffer랑 view는 사라진다.
	//Ptr이기 때문에 nullptr 호출하면 Release()가 호출된다.
	m_SB = nullptr;
	m_SRV = nullptr;
	m_UAV = nullptr;
	...
	// SRV는 texture뿐만 아니라 구조화버퍼 전달용으로도 가능하다.
	// ViewDimension : 이 view의 사용용도

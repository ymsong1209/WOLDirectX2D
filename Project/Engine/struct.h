...
{
	Vec4 vDiffuse;	// 빛의 색상
	Vec4 vAmbient;	// 주변 광(환경 광), 최소한 존재할 수 있는 빛
};
...
struct tLightInfo
{
	...
	Vec4		vWorldPos;   // 광원의 월드 스페이스 위치, finaltick에서 transform의 worldpos가져옴
	Vec4		vWorldDir;	 // 빛을 보내는 방향, finaltick에서 transform의 right벡터 가져옴
...
extern tTransform g_transform; //모든 cpp에서 쓸수있는 전역변수, 쓸라면 extern.cpp에서 선언해줘야함.
...
struct tGlobal
{
	Vec2  Resolution;		//CDevice에서 설정
	float tDT;				//TimeMgr에서 설정

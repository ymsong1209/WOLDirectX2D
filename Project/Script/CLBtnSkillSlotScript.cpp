...
CLBtnSkillSlotScript::CLBtnSkillSlotScript()
	:CScript((UINT)SCRIPT_TYPE::LBTNSKILLSLOTSCRIPT)
	, m_bUIActive(false)
...
void CLBtnSkillSlotScript::begin()
{
...
void CLBtnSkillSlotScript::tick()
{
	if (m_bUIActive) {
		...
		PLAYER_SKILL LBtnSkill = PlayerMainScript->GetSkillPerBtn(0);
		switch (LBtnSkill)
		{
...
void CLBtnSkillSlotScript::SaveToLevelFile(FILE* _File)
{
}

void CLBtnSkillSlotScript::LoadFromLevelFile(FILE* _File)
{
	MeshRender()->GetDynamicMaterial();
...
void CLBtnSkillSlotScript::Activate()
{
	m_bUIActive = true;
	CGameObject* Player = CLevelMgr::GetInst()->GetCurLevel()->FindParentObjectByName(L"Player");
	CPlayerScript* PlayerMainScript = Player->GetScript<CPlayerScript>();
	PLAYER_SKILL ESkill = PlayerMainScript->GetSkillPerBtn(0);
	switch (ESkill)
	{
	case PLAYER_SKILL::NORMALATTACK: {
		m_pCurSkillTex = m_pNormalAttack;
...
void CLBtnSkillSlotScript::Deactivate()
{
	m_bUIActive = false;
	MeshRender()->GetMaterial()->SetTexParam(TEX_0, nullptr);
	m_pCurSkillTex = nullptr;
